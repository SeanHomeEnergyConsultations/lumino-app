  ...
  return (
    <div className="absolute left-4 top-4 z-20 hidden items-center gap-2 xl:flex">
      <button
        ...
        className="app-glass-button app-focus-button flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-slate-700 shadow-panel transition hover:bg-white/90"
      ...
      >
        ...
        {isResultsPanelVisible ? "Hide Targets" : "Show Targets"}
      </button>
      <button
        type="button"
      ...
      >
        ...
        {isDrawerVisible ? "Hide Property Panel" : "Show Property Panel"}
      </button>
    </div>
  );
}
...
export function MapStatusOverlay({
  ...
  return (
    <div className="app-glass-button absolute bottom-24 left-4 right-4 rounded-2xl px-4 py-3 text-sm font-medium text-slate-600 shadow-panel sm:right-auto sm:rounded-full sm:py-2 xl:bottom-4">
      {isSavingVisit
        ? "Saving the field update..."
        : isResolvingTap
          ? "Opening property record..."
          : activeRoute
            ? "Route locked in. Work the next stop, log the outcome, and keep your momentum."
            : routeSelectionMode
              ? "Route builder is live. Tap lead-ready pins to stack a cleaner run."
              : "Sweep the map, open the strongest target, and keep the day moving."}
    </div>
  );
}
...
export function MapCenterLocationButton({
  ...
  return (
    <button
      type="button"
      onClick={onCenter}
      className="app-glass-button app-focus-button absolute bottom-40 right-4 flex h-11 w-11 items-center justify-center rounded-full text-slate-700 shadow-panel transition hover:bg-white/90 sm:bottom-20"
  ...
  return (
    <>
      ...
      <button
        ...
        className="app-glass-button app-focus-button absolute left-4 top-4 flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-slate-700 shadow-panel xl:hidden"
        ...
        <div className="fixed inset-0 z-30 bg-slate-950/20 xl:hidden" onClick={onCloseResults}>
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.2em] text-mist">Targets In View</div>
                <div className="mt-1 text-sm text-slate-600">{items.length} properties in the current map window</div>
              </div>
              <button
                type="button"
                onClick={onCloseResults}
                className="app-glass-button app-focus-button flex h-9 w-9 items-center justify-center rounded-full text-slate-600"
