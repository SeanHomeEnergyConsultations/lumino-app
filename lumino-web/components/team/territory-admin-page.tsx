...
export function TerritoryAdminPage() {
  ...
  async function handleCreateTeam() {
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : "Failed to create team.");
      setTeamState("error");
    }
  }
  ...
  async function handleUpdateTeam(teamId: string) {
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : "Failed to update team.");
      setTeamState("error");
    }
  }
          ...
          <div className="mt-4 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
              ...
              <button
                ...
                disabled={!accessToken || teamState === "saving" || !createTeamName.trim()}
                ...
                <div key={team.teamId} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                  <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                    ...
                    <button
                      ...
                      disabled={!accessToken || teamState === "saving" || !(teamDrafts[team.teamId]?.name ?? team.name).trim()}
                      ...
                      <div className="grid gap-3 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.9fr)_auto] xl:items-center">
                        ...
                        <div>
                          {canAssignTeam ? (
                            ...
                            <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 px-3 py-3 text-sm text-slate-500">
                              {member.role === "manager"
                                ? "Managers own teams rather than being assigned into one."
                                : "Team assignment is only needed for reps and setters."}
                            </div>
                          )}
                        </div>
